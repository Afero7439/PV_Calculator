...
#from fpdf.enums import FPDF.XPos, FPDF.YPos
import os.path
import base64
import itertools
import pickle
...
def create_table(table_data, title='', data_size = 10, title_size=12, align_data='L', align_header='L', cell_width='even', x_start='x_default',emphasize_data=[], emphasize_style=None, emphasize_color=(0,0,0)):
    ...
    # add title
    if title != '':
        pdf.multi_cell(0, line_height, title, border=0, align='j', new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.TOP, max_line_height=pdf.font_size)
    ...
    if  not isinstance(col_width, list):
        if x_start:
            pdf.set_x(x_start)
        for datum in header:
            pdf.multi_cell(col_width, line_height, datum, border=0, align=align_header,  new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.TOP, max_line_height=pdf.font_size)
            x_right = pdf.get_x()
        pdf.ln(line_height) # move cursor back to the left margin
        y2 = pdf.get_y()
        ...
        for row in data:
            ...
            for datum in row:
                if datum in emphasize_data:
                    ...
                    pdf.multi_cell(col_width, line_height, datum, border=0, align=align_data,  new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.TOP, max_line_height=pdf.font_size)
                    pdf.set_text_color(0,0,0)
                    pdf.set_font(style=default_style)
                else:
                    pdf.multi_cell(col_width, line_height, datum, border=0, align=align_data,  new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.TOP, max_line_height=pdf.font_size) # ln = 3 - move cursor to right with same vertical offset # this uses an object named pdf
            pdf.ln(line_height) # move cursor back to the left margin
        ...
        for i in range(len(header)):
            datum = header[i]
            pdf.multi_cell(col_width[i], line_height, datum, border=0, align=align_header,  new_x=FPDF.Xpos.RIGHT, new_y=FPDF.YPos.LAST, max_line_height=pdf.font_size)
            x_right = pdf.get_x()
        pdf.ln(line_height) # move cursor back to the left margin
        y2 = pdf.get_y()
        pdf.line(x_left,y1,x_right,y1)
        ...
        for i in range(len(data)):
            ...
            for i in range(len(row)):
                ...
                if datum in emphasize_data:
                    pdf.set_text_color(*emphasize_color)
                    pdf.set_font(style=emphasize_style)
                    pdf.multi_cell(adjusted_col_width, line_height, datum, border=0, align=align_data,  new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.TOP, max_line_height=pdf.font_size)
                    pdf.set_text_color(0,0,0)
                    pdf.set_font(style=default_style)
                else:
                    pdf.multi_cell(adjusted_col_width, line_height, datum, border=0, align=align_data,  new_x=FPDF.Xpos.RIGHT, new_y=FPDF.YPos.LAST, max_line_height=pdf.font_size) # ln = 3 - move cursor to right with same vertical offset # this uses an object named pdf
            pdf.ln(line_height) # move cursor back to the left margin
    y3 = pdf.get_y()
...
#_____________________________________________________________________________________________________________________________________________________   
    with tab0:
        ...
        with dtac4:
            ...
            design_u = st.text_input("Unit Price", round(design_price*resell,2), disabled=True, key="design_u", label_visibility="hidden")
            commissioning_u = st.text_input("Unit Price", round(commissioning_price*resell,2), disabled=True, key="commissioning_u", label_visibility="hidden")
        with dtac5:
    ...
    with tab6:
        ...
        with f3:
            ...
            if need_grounding_system:
                ...
                separator_price= st.number_input("Buy Price", value=12.0, step=0.5, key = "separator_price", label_visibility="hidden")
...
#calculate costs|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
    if calculate:
        ...
        pdf.cell(200, 16, txt="", new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="L")
        pdf.cell(200, 6, txt="M.A.R.S.T. S.A.", new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="L")
        pdf.cell(200, 6, txt="Tg-Jiu, 210233, Romania", new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="L")
        pdf.cell(200, 6, txt="Str. Termocentralei 2", new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="L")
        pdf.set_font("Times", size=16)
        pdf.cell(200, 6, txt="Quotation for " + str(totalkWp) + " kWp PV system", new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="C")
        pdf.set_font("Times", size=10)
        pdf.cell(200, 6, txt="", new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="L")
        pdf.cell(200, 6, txt="Date: "+ str(date), new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="L")
        pdf.cell(200, 6, txt="Beneficiary: "+ beneficiary, new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="L")
        pdf.cell(200, 6, txt="Address: "+ address,new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="L")
        pdf.cell(200, 6, txt=connection_edit, new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="L")
        pdf.cell(200, 6, txt="Available for 30 days", new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="L")
        ...
        pdf.cell(200,30, txt="",new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="L")
        pdf.cell(200, 6, txt="Signature ________________________", new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="L")
...
# EXTENDED CALCUALTION|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
        ...
        pdf.cell(200, 6, txt="Available for 30 days", new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="L")
        pdf.set_font("Times", size=16)
        pdf.cell(200, 6, txt="Equipment and materials  for a " + str(totalkWp) + " kWp PV system", new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="C")
        pdf.set_font("Times", size=10)
        pdf.cell(200, 6, txt="", new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="L")
        #pdf.cell(200, 6, txt="Resell percentage added: "+str(resell_price)+ ' %', new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="L")
        ...
        pdf.cell(200, 6, txt="*Note: Small material costs are not presented here.", new_x=FPDF.Xpos.LEFT, new_y=FPDF.YPos.NEXT, align="L")
        if checkbox:
